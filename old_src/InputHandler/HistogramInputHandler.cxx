...
TH1* HistoInputHandler::GetHistogram(int i) {
  if (size_t(i) >= fHistos.size()) {
    THROW("Requested histogram, index " << i << ", but only specified "
...
std::vector<TH1*> HistoInputHandler::GetHistograms(int i, int j) {
  ...
  if (j <= i) {
    THROW(
        "Lower bound of GetHistograms range is larger than or equal to the "
        "upper bound: ["
        << i << ", " << j << "].");
  }
...
HistoInputHandler::HistoInputHandler(std::string const& handle,
  ...
  LOG(SAM) << "Creating HistoInputHandler : " << handle << std::endl;
