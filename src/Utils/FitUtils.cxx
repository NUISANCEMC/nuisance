...
double FitUtils::Eavailable(FitEvent *event) {
  ...
  // For CC
  if (event->IsCC()) {
    if (!event->GetHMFSParticle(ISPDG + ((ISPDG < 0) ? 1 : -1))) {
      return -999;
    }
    q0 -= event->GetHMFSParticle(ISPDG + ((ISPDG < 0) ? 1 : -1))->fP.E();
  } else {
    if (!event->GetHMFSParticle(ISPDG)) {
      return -999;
    }
...
double FitUtils::Get_STV_dpt(FitEvent *event, double ProtonMinCut,
...
double FitUtils::Get_STV_dphit(FitEvent *event, double ProtonMinCut,
                               double ProtonMaxCut, double ProtonCosThetaCut,
                               int ISPDG, bool Is0pi) {
  // Check that the neutrino exists
  if (event->NumISParticle(ISPDG) == 0) {
...
double FitUtils::Get_STV_dalphat(FitEvent *event, double ProtonMinCut,
                                 double ProtonMaxCut, double ProtonCosThetaCut,
                                 int ISPDG, bool Is0pi) {
  // Check that the neutrino exists
  if (event->NumISParticle(ISPDG) == 0) {
...
// Returns in GeV
double FitUtils::Get_pn_reco_C(FitEvent *event, double ProtonMinCut,
                               double ProtonMaxCut, double ProtonCosThetaCut,
                               int ISPDG, bool Is0pi) {
...
double FitUtils::Get_pn_reco_Ar(FitEvent *event, double ProtonMinCut,
                                double ProtonMaxCut, double ProtonCosThetaCut,
                                int ISPDG, bool Is0pi) {
