...
void T2K_CC1pip_H2O_XSec_1Dcosmu_nu::SetDataValues(std::string fileLocation) {
  ...
  dataHist = (TH1D*)(dataFile->Get("MuCos/hResultTot"))->Clone();
  //dataHist->Scale(1E-38); //release is in 10^-38 units!
...
// Override this for now
// Should really have Measurement1D do this properly though
void T2K_CC1pip_H2O_XSec_1Dcosmu_nu::SetCovarMatrix(std::string fileLocation) {
  std::cout << "Covariance: " << fileLocation.c_str() << std::endl;
  TFile *dataFile = new TFile(fileLocation.c_str()); //truly great .root file!

  TH2D *covarMatrix = (TH2D*)(dataFile->Get("MuCos/TotalCovariance"))->Clone();
  ...
  for (int i = 1; i < nBinsX+1; i++) {
    for (int j = 1; j < nBinsY+1; j++) {
      (*this->covar)(i-1, j-1) = covarMatrix->GetBinContent(i, j)*1E78; //adds syst+stat covariances
      (*this->fullcovar)(i-1, j-1) = covarMatrix->GetBinContent(i, j)*1E78; //adds syst+stat covariances
    }
  ...
  *this->covar *= 1E78*1E-76; // 1E-76 comes from method in FitUtils::GetChi2FromCov where 1E76 is multiplied out...
