...
T2K_CC1pip_H2O_XSec_1Dcospi_nu::T2K_CC1pip_H2O_XSec_1Dcospi_nu(std::string inputfile, FitWeight *rw, std::string  type, std::string fakeDataFile){
  ...
  measurementName = "T2K_CC1pip_H2O_XSec_1Dcospi_nu";
  plotTitles = "; cos#theta_{#pi}; d#sigma/dcos#theta_{#pi} (cm^{2}/nucleon)";
  EnuMin = 0.;
  EnuMax = 10.;
  isDiag = false;
...
void T2K_CC1pip_H2O_XSec_1Dcospi_nu::SetDataValues(std::string fileLocation) {
  ...
  dataHist = (TH1D*)(dataFile->Get("PosPionCos/hResultTot"))->Clone();
...
void T2K_CC1pip_H2O_XSec_1Dcospi_nu::SetCovarMatrix(std::string fileLocation) {
  std::cout << "Covariances: " << fileLocation.c_str() << std::endl;
  TFile *dataFile = new TFile(fileLocation.c_str()); //truly great .root file!

  TH2D *covarMatrix = (TH2D*)(dataFile->Get("PosPionCos/TotalCovariance"))->Clone();
