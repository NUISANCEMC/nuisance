...
void T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform::FillEventVariables(
    FitEvent *event) {
...
void T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform::FillMCSlice(double x, double y,
                                                           double z, double w) {

  if (z >= -1.0 and z < -0.3)
    fMCHist_Slices[0]->Fill(y, x, w);
  else if (z >= -0.3 and z < 0.3)
    fMCHist_Slices[1]->Fill(y, x, w);
  else if (z >= 0.3 and z < 0.8)
    fMCHist_Slices[2]->Fill(y, x, w);
  else if (z >= 0.8 and z < 1.0)
    fMCHist_Slices[3]->Fill(y, x, w);
}
...
void T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform::SetHistograms() {
  ...
  fInputFile = new TFile(
      (FitPar::GetDataBase() + "/T2K/CC0pi/STV/multidif_3D_pcoscos.root")
          .c_str(),
      "READ");
  // fInputFile->ls();
  ...
  fMCHist_Fine2D = new TH2D("T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_Fine2D",
                            "T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_Fine2D",
  ...
  linearResult->SetNameTitle("T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_data",
  ...
  for (int i = 0; i < 4; i++) { // both y and z slices
    ...
    fDataHist_Slices.push_back(
        (TH2Poly *)fInputFile->Get(Form("dataslice_%i", i))->Clone());
    fDataHist_Slices[i]->SetNameTitle(
        Form("T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_data_Slice%i", i),
        (Form("T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_data_Slice%i", i)));

    for (int j = 0; j < fDataHist_Slices[i]->GetNumberOfBins(); j++) {
      fDataHist_Slices[i]->SetBinError(
          j, sqrt(tempcov->GetBinContent(bincount, bincount)));
    ...
    //        fDataHist_Slices[i]->SetBinError(j+1, k+1,
    //        sqrt((*fFullCovar)(bincount,bincount)) * 1E-38);
    //
    //        std::cout << "Setting data hist " <<
    //        fDataHist_Slices[i]->GetBinContent(j+1,k+1) << " " <<
    //        fDataHist_Slices[i]->GetBinError(j+1,k+1) << std::endl;
    //        fDataHist->SetBinContent(bincount+1,
    //        fDataHist_Slices[i]->GetBinContent(j+1,k+1) );
    //        fDataHist->SetBinError(bincount+1,
    //        fDataHist_Slices[i]->GetBinError(j+1,k+1) );
    //
    //        bincount++;
    //      }
    //    }
    ...
    fMCHist_Slices[i]->SetNameTitle(
        Form("T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_MC_Slice%i", i),
        (Form("T2K_CC0pi1p_XSec_3DPcoscos_nu_nonuniform_MC_Slice%i", i)));
