...
//********************************************************************
T2K_CC0pinp_ifk_XSec_3Dinfa_nu::T2K_CC0pinp_ifk_XSec_3Dinfa_nu(
    nuiskey samplekey) {
  //********************************************************************
...
void T2K_CC0pinp_ifk_XSec_3Dinfa_nu::FillEventVariables(FitEvent *event) {
  ...
  double delta_a = acos(CosThetaP) * 180 / TMath::Pi() -
                   acos(infp_CosThetaP) * 180 / TMath::Pi();

  // std::cout << "CosTheta P is: " << CosThetaP << std::endl;
  // std::cout << "Inferred CosTheta P is: " << infp_CosThetaP << std::endl <<
  // std::endl;
  ...
  /*
    ...
    //std::cout << "Inferred Theta P is: " << tp_inf.Theta() << std::endl <<
    std::endl;

    double delta_a = (Pp.Vect()).Theta()*180/TMath::Pi() -
    tp_inf.Theta()*180/TMath::Pi();
...
void T2K_CC0pinp_ifk_XSec_3Dinfa_nu::ConvertEventRates() {
  ...
  fMCHist->Reset();
  // The first bin in the histogram in underflow, so set this and start bincount
  // at 1
...
void T2K_CC0pinp_ifk_XSec_3Dinfa_nu::FillMCSlice(double x, double y, double z,
                                                 double w) {
  // x is delta_a
  // y is pmu
  // z is CosThetaMu
  ...
  else if (z >= -0.6 and z < 0.0 and y < 0.25)
    fMCHist_Slices[1]->Fill(x, w);
  else if (z >= -0.6 and z < 0.0 and y > 0.25)
    fMCHist_Slices[2]->Fill(x, w);
  else if (z >= 0.0 and y < 0.25)
    fMCHist_Slices[3]->Fill(x, w);
  else if (z >= 0.0 and z < 0.8 and y >= 0.25)
    fMCHist_Slices[4]->Fill(x, w);
  else if (z >= 0.8 and z < 1.0 and y >= 0.25 and y < 0.75)
    fMCHist_Slices[5]->Fill(x, w);
  else if (z >= 0.8 and z < 1.0 and y >= 0.75)
...
void T2K_CC0pinp_ifk_XSec_3Dinfa_nu::SetHistograms() {
  ...
  fDataHist->SetNameTitle("T2K_CC0pinp_ifk_XSec_3Dinfa_nu_data",
                          "T2K_CC0pinp_ifk_XSec_3Dinfa_nu_data");
  ...
  for (int i = 0; i < 7; i++) { // both y and z slices
    ...
    fDataHist_Slices.push_back(
        (TH1D *)fInputFile->Get(Form("resultBin%i_a", i))->Clone());
    fDataHist_Slices[i]->SetNameTitle(
        Form("T2K_CC0pinp_ifk_XSec_3Dinfa_nu_data_Slice%i", i),
        (Form("T2K_CC0pinp_ifk_XSec_3Dinfa_nu_data_Slice%i", i)));

    // Make MC Clones
    fMCHist_Slices.push_back((TH1D *)fDataHist_Slices[i]->Clone());
    fMCHist_Slices[i]->SetNameTitle(
        Form("T2K_CC0pinp_ifk_XSec_3Dinfa_nu_MC_Slice%i", i),
        (Form("T2K_CC0pinp_ifk_XSec_3Dinfa_nu_MC_Slice%i", i)));
