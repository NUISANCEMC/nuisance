...
T2K_CC0pinp_ifk_XSec_3Dinfa_nu::T2K_CC0pinp_ifk_XSec_3Dinfa_nu(
  ...
  // This sample corresponds to the #Delta#theta variable
  // #Delta#theta = #theta_{p}^{measured} - #theta_{p}^{inferred}

  // Setup common settings
  fSettings = LoadSampleSettings(samplekey);
  fSettings.SetDescription(descrip);
  fSettings.SetXTitle("#Delta #theta_{p};p_{#mu}-cos#theta_{#mu}");
  fSettings.SetYTitle("d^{2}#sigma/d#Delta#theta dp_{#mu}dcos#theta_{#mu} (cm^{2}/GeV)");
...
void T2K_CC0pinp_ifk_XSec_3Dinfa_nu::SetHistograms() {
  ...
  for (int i = 0; i < 7; i++) { // both y and z slices
	...
	Form("%s_data_Slice%i", name.c_str(), i),
        Form("%s_data_Slice%i;#Delta #theta_{p};%s", 
	...
	Form("%s_MC_Slice%i", name.c_str(), i),
        Form("%s_MC_Slice%i;#Delta #theta_{p};%s", 
	     name.c_str(), i, fSettings.GetYTitle().c_str()));
