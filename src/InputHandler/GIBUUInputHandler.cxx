...
GIBUUInputHandler::GIBUUInputHandler(std::string const& handle,
  ...
  TH1D* eFlux = dynamic_cast<TH1D*>(rootFile->Get("e_flux"));
  ...
  if (eFlux) {
    eFlux = static_cast<TH1D*>(eFlux->Clone());
    eFlux->Scale(1.0 / eFlux->Integral("width"));
    std::cout << "GiBUU Flux: eFlux, Width integral = "
              << eFlux->Integral("width") << std::endl;
    eFlux->SetDirectory(NULL);
    eFlux->SetNameTitle((fName + "_e_FLUX").c_str(),
                        (fName + "; E_{#nu} (GeV); #Phi_{e} (A.U.)").c_str());
    fFluxList.push_back(eFlux);
  }
  rootFile->Close();
  ...
  if (!IsEScattering) {
    ...
    LOG(SAM) << "Looking for dominant vector species in GiBUU file ("
             << rawinputs
             << ") expecting to find: " << (eFlux ? "e_Flux" : expectStr)
             << std::endl;

    if (maskHW > 2) {
      LOG(SAM)
          << "We are looking for more than two species... this will have to "
      ...
      switch (isnu->fPID) {
        case 12: {
          ...
          LOG(SAM) << "\tGiBUU File: " << rawinputs << " -- ev: " << nevt
                   << " has IS nu (" << isnu->fPID
                   << "), species weight: " << fGiReader->SpeciesWght
                   << std::endl;
        ...
        case -14: {
          ...
          flux = numubFlux;
          LOG(SAM) << "\tInput file: " << rawinputs
                   << " determined to be numubar dominated vector."
                   << std::endl;
          break;
        }
        default: {}
  ...
  if (!flux) {
    ERR(FTL) << "Couldn't find flux("
             << (IsEScattering ? "e_Flux"
  ...
  TH1D* xsechist =
      static_cast<TH1D*>(eventhist->Clone((fName + "_XSEC").c_str()));
