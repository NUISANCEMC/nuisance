...
int main(){
  ...
  for(size_t i = 0; i < 3; ++i){
    std::cout << "\tTest param name: " << pnames[i] << " == " << min.GetParamName(i) << std::endl;
    assert(pnames[i]==min.GetParamName(i));
    std::cout << "\tTest param post-fit value : fabs(" << postfitvals[i] << " - " << min.GetParamVal(i) << ") < " << min.GetParamError(i)  << std::endl;
    assert(fabs(postfitvals[i] - min.GetParamVal(i)) < min.GetParamError(i));
    std::cout << "\tTest post-fit error " << min.GetParamError(i) << " < " << FakeDataError << std::endl;
    assert(min.GetParamError(i) < FakeDataError);
  }
