...
SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu(std::string name, std::string inputfile,
  ...
  this->SetDataValues(FitPar::GetDataBase()+"/SciBooNE/SB_COH_Fig10d_CVs.csv");
...
bool SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::isSignal(FitEvent *event){
  ...
  for (UInt_t j = 2; j < event->Npart(); j++){
    ...
    if (abs(PID) == 211 || PID == 2212){
      ...
      if (SciBooNEUtils::PassesCOHDistanceCut(event->PartInfo(0), event->PartInfo(j))){
	nCharged += 1;
	if (PID == 211) nPions += 1;
      } else nVertex += 1;
    }
...
void SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::ScaleEvents(){
...
void SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::FillHistograms(){
...
void SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::Write(std::string drawOpt){
...
void SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::ApplyNormScale(double norm){
...
void SciBooNE_CCCOH_MuPiNoVA_1DQ2_nu::ResetAll(){
